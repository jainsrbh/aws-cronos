...
locals {
  ...
  ec2_configuration = local.environment_vars.locals.ec2.jump
...
inputs = {
  name                 = "${local.vertical}-${local.env}-jump"
  ami                  = dependency.ami.outputs.ami_id
  availability_zone    = local.availability_zone
  subnet_id            = dependency.network.outputs.public_subnets_ids_full["${local.availability_zone}"]
  ...
  vpc_security_group_ids = [
    dependency.security_groups.outputs.security_groups.jump.id
  ]
  user_data = templatefile(local.user_data_file, merge({
    key_pair_jump  = dependency.key_jump.outputs.private_key_pem
    user     = "ec2-user"
  }))
